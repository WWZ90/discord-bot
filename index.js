...
*/
  ...
  async function getGoogleDoc() {
    ...
    console.log(
      "Initializing GoogleSpreadsheet instance for the first time...",
    );
    if (GOOGLE_SERVICE_ACCOUNT_EMAIL && GOOGLE_PRIVATE_KEY && GOOGLE_SHEET_ID) {
      try {
        ...
        console.error(
          "Error creating JWT or GoogleSpreadsheet instance:",
          error,
        );
...
*/
      ...
      if (blockTypes.includes(recordType)) {
        ...
        if (!closingMessage) {
          ...
          for (const msg of messagesAfterTicketTool) {
            ...
            if (
              msg.author.bot &&
              !KNOWN_VERIFIER_BOT_IDS.includes(msg.author.id)
            )
              continue;
  ...
  async function processThread(
    ...
    try {
      ...
      if (blockTypes.includes(recordType)) {
        ...
        console.log(
          `${logPrefix} Processing as ${typeColumn} (manual command).`,
        );
      } else {
        closingMessage = manualClosingMessage;
        let autoClosingGenerated = false;
        ...
        if (!closingMessage) {
          ...
          console.log(
            `${logPrefix} No manual CLOSING. Attempting auto-closing.`,
          );
        ...
        if (closingMessage) {
          ...
          if (blockTypes.includes(closingLineContentLower)) {
            ...
            usersRaw.forEach((userStr) => {
              const findoorMatch = userStr.match(
                /^(.*?)\s*\(\s*findoor\s*\)$/i,
              );
              if (findoorMatch) {
  ...
  async function performMassScan() {
    ...
    try {
      ...
      console.log(
        `[${new Date().toISOString()}] Scanning guild: ${guild.name}`,
      );
  ...
  client.on("messageCreate", async (message) => {
    if (message.channel.id === OO_LIVE_FEED_CHANNEL_ID && message.author.bot) {
      ...
      if (marketLink) {
        const transactionHashMatch = marketLink.match(
          /transactionHash=([^&]+)/,
        );
    ...
    if (commandName === "record") {
        ...
        for (let i = 0; i < allThreads.length; i++) {
          ...
          try {
            ...
            if (shouldArchive && !fetchedThread.archived) {
              await fetchedThread.setArchived(
                true,
                "Backlog processing cleanup",
              );
            }
          } catch (err) {
            failedCount++;
      ...
      try {
        ...
        console.error(
          "[Unarchive] Major error during unarchive command:",
          error,
        );
        await message.channel.send(
      ...
      try {
        ...
        const verificationsSheet =
          googleDoc.sheetsByTitle[WORKSHEET_TITLE_MAIN];
        ...
        console.log(
          "[Duplicates] Fetching all OO Links from Findoors sheet...",
        );
        ...
        console.error(
          "[Duplicates] Error during findduplicates command:",
          error,
        );
        await message.channel.send(
      ...
      try {
        ...
        console.log(
          `[FixDuplicates] Loaded ${findoorLinks.size} unique links.`,
        );
    ...
    if (channel.name.toLowerCase().startsWith("proposal-")) {
      ...
      setTimeout(async () => {
        try {
          ...
          for (const msg of messages.values()) {
            if (
              msg.author.id === TICKET_TOOL_USER_ID &&
              msg.embeds.length > 0
            ) {
          ...
          if (txHashMatch?.[1] && eventIndexMatch?.[1]) {
            ...
            if (createdFallbackThreads.has(uniqueId)) {
              ...
              try {
                ...
                console.error(
                  `${logPrefix} Error closing duplicate ticket:`,
                  e,
                );
              }
  ...
  async function initializeBot() {
    ...
    if (!process.env.DISCORD_TOKEN || !process.env.GOOGLE_SHEET_ID) {
      console.error(
        "CRITICAL: DISCORD_TOKEN or GOOGLE_SHEET_ID is missing. Bot will not start.",
      );
      return;
    }
    ...
    client.on("ready", () => {
      console.log(
        `✅ ${client.user.tag} has connected to Discord and is ready!`,
      );

      loadConfig().then(() => {
        console.log("Configuration loaded successfully after bot ready.");

        if (botConfig.autoProcessingEnabled) {
          console.log(
            "Auto-processing is enabled. Performing an initial scan after a short delay.",
          );
        ...
        console.log(
          `[Supervisor] Fallback queue processor started. Checking every ${FALLBACK_QUEUE_PROCESS_INTERVAL_MS / 1000} seconds.`,
        );
      });
    });
  ...
  // Si algo falla durante la carga inicial, lo veremos aquí.
